...
open class SKPhoto: NSObject, SKPhotoProtocol {
    ...
    open func checkCache() {
        ...
        if SKCache.sharedCache.imageCache is SKRequestResponseCacheable {
            ...
            if let imgData:Data = SKCache.sharedCache.imageDataForRequest(request) {
                
                underlyingImage = UIImage.sd_animatedGIF(with: imgData)
            }
            
        } else {
            
            if let imgData = SKCache.sharedCache.imageDataForKey(photoURL) {
    ...
    open func loadUnderlyingImageAndNotify() {
        ...
        let session = URLSession(configuration: URLSessionConfiguration.default)
            ...
            task = session.dataTask(with: URL, completionHandler: { [weak self] (data, response, error) in
                ...
                if let data = data, let response = response, let image = UIImage.sd_animatedGIF(with: data) {
                    if self.shouldCachePhotoURLImage {
                        if SKCache.sharedCache.imageCache is SKRequestResponseCacheable {
                            ...
                            SKCache.sharedCache.setImageData(data, forKey: self.photoURL)
                        }
                    }
                    DispatchQueue.main.async {
