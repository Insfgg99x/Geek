...
open class SKPhotoBrowser: UIViewController {
    ...
    var canLongPressSaveImage: Bool = true
    ...
    func setup() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(userDidLongPressImage(_:)), name: NSNotification.Name.init(SKPhotoBrowserLongPressImageNotification), object: nil)
    }
    ...
    @objc func userDidLongPressImage(_ sender:Notification){
        
        guard canLongPressSaveImage else {
            
            return
        }
        let imageView = sender.object as! SKDetectingImageView
        guard (imageView.image != nil) else {
            
            return
        }
        let sheet = UIAlertController.init(title: "请选择", message: nil, preferredStyle: .actionSheet)
        sheet.addAction(UIAlertAction.init(title: "保存至相册", style: .default, handler: { (sender) in
            let image=imageView.image
            UIImageWriteToSavedPhotosAlbum(image!, self, #selector(self.image(image:didFinishSavingWithError:contextInfo:)), nil)
        }))
        sheet.addAction(UIAlertAction.init(title: "取消", style: .default, handler: nil))
